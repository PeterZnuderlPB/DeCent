...
class PBModel(models.Model):
    user_created = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="%(app_label)s_%(class)s_related_UserCreated", on_delete=models.CASCADE, blank=True)
    date_created = models.DateTimeField(auto_now_add=True)
    user_last_modified = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="%(app_label)s_%(class)s_related_UserLastModified", on_delete=models.CASCADE, blank=True)
